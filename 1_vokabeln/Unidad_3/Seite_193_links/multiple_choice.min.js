<!DOCTYPE html>
<html lang="de">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Multiple-Choice Quiz: Spanisch Vokabeln</title>
    <style>
        /* Grundlegende Seiten-Einstellungen */
        body {
            font-family: Arial, sans-serif;
            display: flex;
            justify-content: center;
            align-items: center;
            flex-direction: column;
            height: 100vh;
            margin: 0;
            background-color: #e6f4ea; /* Einheitliches Pastellgrün */
        }

        /* Header Styling */
        header {
            width: 100%;
            background-color: #82c8a0; /* Einheitliches Pastellgrün */
            color: white;
            text-align: center;
            padding: 20px;
            box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);
        }

        header h1 {
            margin: 0;
            font-size: 1.8em;
        }

        /* Fragebereich und Fortschrittsanzeige */
        #questionContainer {
            width: 400px;
            min-height: 150px;
            background-color: #82c8a0; /* Einheitliches Pastellgrün */
            border-radius: 10px;
            box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);
            display: flex;
            flex-direction: column;
            align-items: center;
            justify-content: center;
            font-size: 1.2em;
            text-align: center;
            padding: 20px;
            margin-bottom: 20px;
            color: white;
        }

        #question {
            font-size: 24px;
            color: #2b2b2b;
        }

        /* Timer- und Fortschrittsanzeige */
        #timer, #points, #progress {
            font-size: 18px;
            color: #3c6e47;
            margin-top: 10px;
        }

        #progress-bar {
            width: 80%;
            height: 20px;
            background-color: #cce3d0;
            border-radius: 10px;
            overflow: hidden;
            margin-top: 20px;
        }

        #progress-fill {
            height: 100%;
            background-color: #5e9c76;
            width: 0%;
            transition: width 0.5s ease;
        }

        /* Antwort-Schaltflächen */
        #choices {
            display: flex;
            flex-direction: column;
            gap: 10px;
            width: 100%;
            margin-top: 20px;
        }

        .choiceButton {
            padding: 10px;
            font-size: 16px;
            cursor: pointer;
            border: none;
            border-radius: 5px;
            background-color: #82c8a0; /* Einheitliches Pastellgrün */
            color: white;
            transition: background-color 0.3s;
            width: 100%;
            box-shadow: 0px 4px 8px rgba(0, 0, 0, 0.1);
        }

        .choiceButton:hover {
            background-color: #6fae8b;
        }
    </style>
    <script>
        document.addEventListener("DOMContentLoaded", () => {
            const questions = [
                { question: "'telefonieren'?", choices: ["hablar por teléfono", "estudiar", "el alemán", "el perro"], correct: "hablar por teléfono" },
                { question: "'jeden Tag'?", choices: ["todos los días", "la escuela", "el nombre", "el idioma"], correct: "todos los días" },
                { question: "'der Sommer'?", choices: ["el verano", "la emoción", "la fiesta", "el amigo"], correct: "el verano" },
                { question: "'verbringen'?", choices: ["pasar", "el centro", "la casa", "el amigo"], correct: "pasar" },
                { question: "'die Ferien'?", choices: ["las vacaciones", "el actor", "el nombre", "la plaza"], correct: "las vacaciones" },
                { question: "'der Name'?", choices: ["el nombre", "el instituto", "la plaza", "la cosa"], correct: "el nombre" },
                { question: "'die Schule'?", choices: ["el instituto", "el móvil", "la plaza", "la casa"], correct: "el instituto" },
                { question: "'der Schauspieler'?", choices: ["el actor", "la actriz", "el chico", "la cosa"], correct: "el actor" },
                { question: "'die Schauspielerin'?", choices: ["la actriz", "el nombre", "el coche", "la plaza"], correct: "la actriz" },
                { question: "'der Hund'?", choices: ["el perro", "el centro", "la casa", "la cosa"], correct: "el perro" },
                { question: "'genial'?", choices: ["genial", "el instituto", "el coche", "el idioma"], correct: "genial" },
                { question: "'Mexiko Stadt'?", choices: ["Ciudad de México", "la costa", "el instituto", "la noche"], correct: "Ciudad de México" },
                { question: "'lernen'?", choices: ["estudiar", "el día", "la plaza", "el amigo"], correct: "estudiar" },
                { question: "'USA'?", choices: ["Estados Unidos", "el chico", "la plaza", "la casa"], correct: "Estados Unidos" },
                { question: "'die Sprache'?", choices: ["el idioma", "la puerta", "el perro", "la cosa"], correct: "el idioma" },
                { question: "'Englisch'?", choices: ["el inglés", "la puerta", "el coche", "la plaza"], correct: "el inglés" },
                { question: "'Deutsch'?", choices: ["el alemán", "la playa", "la plaza", "el chico"], correct: "el alemán" },
                { question: "'welche Sprachen sprichst du?'?", choices: ["¿qué idiomas hablas?", "todos los días", "genial", "el amigo"], correct: "¿qué idiomas hablas?" }
            ];

            const totalQuestions = questions.length;
            let currentQuestionIndex = 0;
            let points = 0;

            // Elements
            const questionContainer = document.getElementById("question");
            const choicesContainer = document.getElementById("choices");
            const pointsDisplay = document.getElementById("points");
            const progressDisplay = document.getElementById("progress");
            const progressBarFill = document.getElementById("progress-fill");

            // Load a question
            function loadQuestion() {
                const questionData = questions[currentQuestionIndex];
                questionContainer.textContent = questionData.question;
                choicesContainer.innerHTML = "";

                // Generate answer buttons
                questionData.choices.forEach(choice => {
                    const button = document.createElement("button");
                    button.classList.add("choiceButton");
                    button.textContent = choice;
                    button.onclick = () => handleAnswer(choice);
                    choicesContainer.appendChild(button);
                });
            }

            // Handle answer selection
            function handleAnswer(selectedChoice) {
                const questionData = questions[currentQuestionIndex];
                if (selectedChoice === questionData.correct) {
                    points++;
                }

                currentQuestionIndex++;

                // Update points and progress
                pointsDisplay.textContent = `Punkte: ${points}`;
                updateProgressBar();

                if (currentQuestionIndex < totalQuestions) {
                    loadQuestion();
                } else {
                    questionContainer.textContent = "Quiz abgeschlossen! Gut gemacht!";
                    choicesContainer.innerHTML = "";
                }
            }

            // Update progress bar based on questions answered
            function updateProgressBar() {
                const progressPercent = (currentQuestionIndex / totalQuestions) * 100;
                progressBarFill.style.width = `${progressPercent}%`;
                progressDisplay.textContent = `Fortschritt: ${currentQuestionIndex} / ${totalQuestions}`;
            }

            // Initialize quiz
            loadQuestion();
            pointsDisplay.textContent = `Punkte: ${points}`;
            updateProgressBar();
        });
    </script>
</head>
<body>
    <header>
        <h1>Multiple-Choice Quiz: Spanisch Vokabeln</h1>
    </header>
    <main>
        <div id="points">Punkte: 0</div>
        <div id="progress">Fortschritt: 0 / 18</div>
        <div id="questionContainer">
            <span id="question">Frage wird geladen...</span>
            <div id="choices"></div>
        </div>
        <div id="progress-bar">
            <div id="progress-fill"></div>
        </div>
    </main>
</body>
</html>
